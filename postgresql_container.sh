...
while true; do
    read -p "This will create a new PostgreSQL 14.x LXC Container. Proceed (y/n)?" yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
wget -qL https://raw.githubusercontent.com/StevenSeifried/proxmox-scripts/main/postgresql_setup.sh
...
pct start $CTID
pct push $CTID postgresql_setup.sh /postgresql_setup.sh -perms 755
pct exec $CTID /postgresql_setup.sh

# Get network details and show completion message
IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')
info "Successfully created a PostgreSQL 14.x LXC Container to $CTID at IP Address ${IP}"
